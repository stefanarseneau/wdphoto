...
def get_model_flux(params, filters, interp = None, key = None):
    ...
    if interp is None:
        ...
        fl = 4 * np.pi * interp(teff, logg)[indx]   
...
def residual(params, obs_flux = None, e_obs_flux = None, filters = None, interp = None, key = None):
    #calculate the chi2 between the model and the fit
    model_flux = get_model_flux(params, filters, interp, key)
    
    chisquare = ((model_flux - obs_flux) / e_obs_flux)**2
    return chisquare

def get_parameters(obs_mag, e_obs_mag, filters, distance, interpolator, p0 = [10000, 8, 0.003]):    
    obs_flux, e_obs_flux = mag_to_flux_spec_Vega(obs_mag, filters, e_obs_mag) # convert magnitudes to fluxes
    interp, key = interpolator.interp, interpolator.key

    #use lmfit.minimize to fit the model to the data
    params = lmfit.Parameters()
    params.add('teff', value = p0[0], min = interpolator.teff_lims[0], max = interpolator.teff_lims[1], vary = True)
    params.add('logg', value = p0[1], min = interpolator.logg_lims[0], max = interpolator.logg_lims[1], vary=False)
    params.add('radius', value = p0[2], min = 0.000001, max = 0.1, vary = True)
    params.add('distance', value = distance, vary = False)
            
    result = lmfit.minimize(residual, params, kws = dict(obs_flux = obs_flux, e_obs_flux = e_obs_flux, filters = filters, interp = interp, key = key), method = 'leastsq')
